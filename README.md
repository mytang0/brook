...
| HTTP metadata | `brook-metadata-http` or Spring `brook-spring-boot-http-metadata` | Set `brook.metadata.protocol=http` and `brook.metadata.http.serverUri`; optional keys include `nameKey`, `versionKey`, `wrapped`, cache settings. |
