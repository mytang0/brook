...
public abstract class IDUtils {
    ...
    private static final int MASK3 = 0x07;

    private static final int MASK4 = 0x0f;

    /**
     * Remove the '-' of uuid, there will be 4 bytes (32 bits) available.
     * Used in hexadecimal, there are 2 bytes (16 bits).
     * <p>
     * 0 ~ 3: Type
     * 4: Local execution
     * 5 ~ 15: Reserved
     */
    public static String generator(final FlowDef flowDef) {
        byte[] head = new byte[2];
        head[0] = Type.FLOW;
        return bytes2hex(head) + UUID.randomUUID().toShortString();
    }

    public static String generator(final TaskDef taskDef) {
        byte[] head = new byte[2];
        head[0] = Type.TASK;
        return bytes2hex(head) + UUID.randomUUID().toShortString();
    }

    public static boolean isFlow(String id) {
        if (id == null) {
            return false;
        }
        byte[] headType = hex2bytes(id, 0, 2);
        return (headType[0] & MASK3) == Type.FLOW;
    }
    ...
    public static boolean isTask(String id) {
        ...
        byte[] headType = hex2bytes(id, 0, 2);
        return (headType[0] & MASK3) == Type.TASK;
    }

    public static boolean isLocalExecution(String id) {
        if (id == null) {
            return false;
        }
        byte[] headType = hex2bytes(id, 0, 2);
        return (headType[0] & 1 << Bit.LOCAL_EXECUTION) != 0;
    }
    ...
    private static String bytes2hex(byte[] bs, int off, int len) {
        if (off < 0) {
    ...
    private static byte[] hex2bytes(final String str, final int off, int len) {
        if ((len & 1) == 1) {
    ...
    private static byte unSetBit(byte b, int bit) {
        b &= ~(1 << bit);
        return b;
    }
     ...
     * @since 1.5
     */
    private static final class UUID implements java.io.Serializable, Comparable<UUID> {
         ...
         * object, has the same variant, and contains the same value, a bit for bit,
         * as this {@code UUID}.
         *
